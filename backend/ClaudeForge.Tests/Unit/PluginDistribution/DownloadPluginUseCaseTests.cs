...
namespace ClaudeForge.Tests.Unit.PluginDistribution;

/// <summary>
/// Unit tests for Group 6 — DownloadPluginUseCase.
///
...
///
///   PluginNotFoundException : ProblemDetailsException
///     Message    == "Plugin not found"
///     StatusCode == 404
///     (Reuse ClaudeForge.Application.Modules.PluginCatalog.UseCases.PluginNotFoundException)
///
...
public sealed class DownloadPluginUseCaseTests
{
    ...
    private static (
        IPluginDistributionRepositoryPort repo,
        IPackageStoragePort storage,
        DownloadPluginUseCase useCase) BuildUseCase(
        DownloadResolutionResult? resolveResult = null)
    {
        IPluginDistributionRepositoryPort repo =
            Substitute.For<IPluginDistributionRepositoryPort>();
        IPackageStoragePort storage =
        ...
        DownloadPluginUseCase useCase = new(repo, storage);
    ...
    public async Task ExecuteAsync_NullVersion_ResolvesAndStreamsLatestVersion()
    {
        ...
        (_, _, DownloadPluginUseCase useCase) =
    ...
    public async Task ExecuteAsync_NullVersion_PassesNullToResolveAsync()
    {
        ...
        (IPluginDistributionRepositoryPort repo, _, DownloadPluginUseCase useCase) =
            BuildUseCase();
    ...
    public async Task ExecuteAsync_OnFound_CallsStorageGetAsyncWithPackageKey()
    {
        ...
        (_, IPackageStoragePort storage, DownloadPluginUseCase useCase) =
            BuildUseCase(new FoundResult(resolution));
    ...
    {
        // Arrange
        (_, _, DownloadPluginUseCase useCase) = BuildUseCase(new PluginNotFoundResult());

        // Act & Assert
        ClaudeForge.Application.Modules.PluginCatalog.UseCases.PluginNotFoundException ex =
            await Assert.ThrowsAsync<ClaudeForge.Application.Modules.PluginCatalog.UseCases.PluginNotFoundException>(

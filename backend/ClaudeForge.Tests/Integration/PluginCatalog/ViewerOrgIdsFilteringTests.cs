...
/// ListPluginsUseCase, GetPluginDetailsUseCase, SearchPluginsUseCase, and
/// DiscoverPluginsUseCase, the viewerOrgIds SQL predicate is correctly applied:
///
...
///   - ListPluginsUseCase gains (ICurrentUser, IOrgMembershipQueryPort) constructor args
///   - IPluginRepositoryPort.ListPluginsAsync gains viewerOrgIds: IReadOnlySet&lt;Guid&gt; arg
///   - IPluginRepositoryPort.GetPluginByIdAsync gains viewerOrgIds: IReadOnlySet&lt;Guid&gt; arg
...
///   - GetPluginDetailsUseCase gains (ICurrentUser, IOrgMembershipQueryPort)
///   - SearchPluginsUseCase + DiscoverPluginsUseCase gain (ICurrentUser, IOrgMembershipQueryPort)
/// </summary>
...
public sealed class ViewerOrgIdsFilteringTests : IAsyncLifetime
{
    ...
    private static PluginEntity MakePublicPlugin(string name) => new()
    {
        Id = Guid.NewGuid(),
        Name = name,
    ...
    private static PluginEntity MakePrivatePlugin(string name, Guid ownerOrgId, Guid? ownerUserId = null) => new()
    {
        Id = Guid.NewGuid(),
        Name = name,
        NameNormalized = name.ToLowerInvariant(),
    ...
    public async Task ListPlugins_Anonymous_SeesOnlyPublicPlugins()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity pub = MakePublicPlugin("anon-list-public");
        PluginEntity priv = MakePrivatePlugin("anon-list-private", orgId);
    ...
    public async Task ListPlugins_Member_SeesPublicPlusOwnOrgPrivate()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity pub = MakePublicPlugin("member-list-public");
        PluginEntity ownPriv = MakePrivatePlugin("member-list-own-private", orgId);
    ...
    public async Task ListPlugins_MemberOfOrgB_DoesNotSeeOrgAPrivate_AndCountIsCorrect()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity pub = MakePublicPlugin("cross-org-list-public");
        PluginEntity orgAPriv = MakePrivatePlugin("orgA-list-private", orgA); // not visible to orgB member
    ...
    public async Task ListPlugins_MultiOrgMember_SeesPrivateFromAllOwnOrgs()
    {
        ...
        PluginEntity pub = MakePublicPlugin("multi-org-public");
        PluginEntity orgXPriv = MakePrivatePlugin("multi-org-x-private", orgX);
        PluginEntity orgYPriv = MakePrivatePlugin("multi-org-y-private", orgY);
    ...
    public async Task GetPluginDetails_PrivatePlugin_NonMember_Returns404()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity priv = MakePrivatePlugin("detail-filter-private", orgId);
    ...
    public async Task GetPluginDetails_PrivatePlugin_Member_Returns200()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity priv = MakePrivatePlugin("detail-member-private", orgId, memberId);
        ctx.Plugins.Add(priv);
        await ctx.SaveChangesAsync();
        await SeedVersionAsync(priv.Id);
    ...
    public async Task GetPluginDetails_PrivatePlugin_Anonymous_Returns404()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity priv = MakePrivatePlugin("detail-anon-private", orgId);
        ctx.Plugins.Add(priv);
        await ctx.SaveChangesAsync();
        await SeedVersionAsync(priv.Id);
    ...
    public async Task SearchPlugins_Anonymous_ExcludesPrivateFromResultsAndCount()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity pub = MakePublicPlugin("search-public-vis");
        pub.Description = "A searchable public plugin for visibility test";
        PluginEntity priv = MakePrivatePlugin("search-private-vis", orgId);
    ...
    public async Task SearchPlugins_Member_SeesOwnOrgPrivateInResults()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity priv = MakePrivatePlugin("search-member-private-vis", orgId);
    ...
    public async Task DiscoverPlugins_Anonymous_ExcludesPrivate()
    {
        ...
        await using MarketplaceDbContext ctx = _fixture.CreateContext();
        PluginEntity pub = MakePublicPlugin("discover-public");
        pub.Description = "Discover public plugin for filtering test";
        PluginEntity priv = MakePrivatePlugin("discover-private", orgId);

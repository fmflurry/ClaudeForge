...
///   - UploadAddOnUseCase gains ICurrentUser + IOrgMembershipQueryPort + IAddOnAccessPolicy
///   - POST /api/v1/plugins/upload gains [Authorize] gate behind Features:RequireAuthForUpload
///   - UploadAddOnCommand gains Visibility + OwnerOrgId + OwnerUserId fields
///   - ChangeAddOnVisibilityUseCase + PATCH /api/v1/plugins/{pluginId}/visibility endpoint exist
///   - IAddOnPublishingRepositoryPort gains UpdateVisibilityAsync(pluginId, visibility, ownerOrgId)
/// </summary>
...
public sealed class UploadAuthGateTests : IAsyncLifetime
{
    ...
    public async Task PostUpload_FlagOn_AuthenticatedPublicUpload_Returns201()
    {
        ...
        AddOnEntity? plugin = await ctx.Plugins.AsNoTracking().FirstOrDefaultAsync(p => p.Id == pluginId);
    ...
    public async Task PatchVisibility_OwnerChangesPrivateToPublic_Returns200AndClearsOwnerOrgId()
    {
        ...
        AddOnEntity? updated = await ctx.Plugins.AsNoTracking()

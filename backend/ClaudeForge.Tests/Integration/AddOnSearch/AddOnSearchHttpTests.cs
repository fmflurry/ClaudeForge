...
public sealed class PluginSearchHttpTests : IAsyncLifetime
{
    ...
    private static AddOnVersionEntity MakeVersion(Guid pluginId, string version, long versionSort) => new()
    {
        Id = Guid.NewGuid(),
        PluginId = pluginId,
        Version = version,
    ...
    public async Task SearchEndpoint_FilterByType_ReturnsOnlyMatchingType()
    {
        ...
        AddOnEntity skillPlugin = MakePlugin("AuthSkillHttp", "auth-skill-http-type",
            "Auth skill plugin");
        AddOnEntity hookPlugin = MakePlugin("AuthHookHttp", "auth-hook-http-type",
            "Auth hook plugin");
    ...
    public async Task SearchEndpoint_FilterByLanguage_ReturnsOnlyMatchingLanguage()
    {
        ...
        AddOnEntity tsPlugin = MakePlugin("ValidationTsHttp", "validation-ts-http",
            "TypeScript validation http test");
        AddOnEntity pyPlugin = MakePlugin("ValidationPyHttp", "validation-py-http",
    ...
    public async Task DiscoveryEndpoint_FilterByLanguage_ReturnsOnlyMatchingLanguage()
    {
        ...
        AddOnEntity tsPlugin = MakePlugin("TsDiscoveryPlugin", "ts-discovery-lang",
            "TypeScript discovery language filter test");
        AddOnEntity pyPlugin = MakePlugin("PyDiscoveryPlugin", "py-discovery-lang",
    ...
    public async Task DiscoveryEndpoint_CombinedCriteria_OnlyPluginsSatisfyingAllConditionsReturned()
    {
        ...
        AddOnEntity matchAll = MakePlugin("PythonTestingDevTeam", "py-testing-dt-combo",
            "Python testing plugin for dev team use case");
        // Matches keyword + language but NOT use case
        AddOnEntity matchTwoOnly = MakePlugin("PythonTestingNoUseCase", "py-testing-no-uc-combo",
            "Python testing plugin without dev-team tag");

        ctx.Plugins.AddRange(matchAll, matchTwoOnly);
        await ctx.SaveChangesAsync();

        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = matchAll.Id, CategoryId = langPy.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = matchAll.Id, CategoryId = useCaseDevTeam.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = matchTwoOnly.Id, CategoryId = langPy.Id });
        await ctx.SaveChangesAsync();
        await RefreshSearchVectors(ctx);

...
namespace ClaudeForge.Tests.Integration.AddOnSearch;
...
///
///   ClaudeForge.Infrastructure.AddOnSearch.PostgresSearchAdapter : ISearchIndexPort
///     PostgresSearchAdapter(MarketplaceDbContext context)
///
///   ClaudeForge.Application.Modules.AddOnSearch.Ports.ISearchIndexPort
...
///
///   ClaudeForge.Application.Modules.AddOnSearch.UseCases.SearchResultDto
///     float RelevanceScore; long DownloadCount; DateTimeOffset CreatedAt; string Name; string Slug
///
///   ClaudeForge.Application.Modules.AddOnSearch.UseCases.SearchCriteria
...
public sealed class SearchIndexPortTests : IAsyncLifetime
{
    ...
    public async Task SearchAsync_NameMatch_ReturnsMatchingPlugin()
    {
        ...
        AddOnEntity authPlugin = MakePlugin("AuthHelper", "auth-helper", "OAuth authentication helper");
        AddOnEntity unrelatedPlugin = MakePlugin("DatabaseUtils", "database-utils", "Database utility functions");
    ...
    public async Task SearchAsync_DescriptionMatch_ReturnsPluginWithMatchingDescription()
    {
        ...
        AddOnEntity plugin = MakePlugin("OAuthValidator", "oauth-validator",
    ...
    public async Task SearchAsync_Ranking_ExactNameMatchRanksBeforePrefixAndPartial()
    {
        ...
        // Exact match
        AddOnEntity exact = MakePlugin("Logger", "logger-exact",
            "A plugin called Logger", downloadCount: 10);
        // Prefix match
        AddOnEntity prefix = MakePlugin("LoggerHelper", "logger-helper",
            "Extends the logger functionality", downloadCount: 5);
        // Partial/description match only
        AddOnEntity partial = MakePlugin("FileWriter", "file-writer",
    ...
    public async Task SearchAsync_Ranking_HigherDownloadCountBreaksTie()
    {
        ...
        AddOnEntity lowDownloads = MakePlugin("TestingPlugin", "testing-low-dl",
            "A testing plugin", downloadCount: 5, createdAt: sameTime);
        AddOnEntity highDownloads = MakePlugin("TestingLib", "testing-high-dl",
    ...
    public async Task SearchAsync_Ranking_MoreRecentPluginBreaksTieByRecency()
    {
        ...
        AddOnEntity oldPlugin = MakePlugin("ValidationHelper", "validation-old",
            "Validates input data", downloadCount: 0, createdAt: older);
        AddOnEntity newPlugin = MakePlugin("ValidationKit", "validation-new",
    ...
    public async Task SearchAsync_TypeFilter_ReturnsOnlyMatchingType()
    {
        ...
        AddOnEntity skillPlugin = MakePlugin("AuthSkill", "auth-skill",
            "Auth skill plugin for authentication");
        AddOnEntity hookPlugin = MakePlugin("AuthHook", "auth-hook",
    ...
    public async Task SearchAsync_MultipleTypeFilters_OrLogicWithinDimension()
    {
        ...
        AddOnEntity skillPlugin = MakePlugin("MultiAuthSkill", "multi-auth-skill",
            "Multi auth skill plugin");
        AddOnEntity hookPlugin = MakePlugin("MultiAuthHook", "multi-auth-hook",
            "Multi auth hook plugin");
        AddOnEntity agentPlugin = MakePlugin("MultiAuthAgent", "multi-auth-agent",
        ...
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = skillPlugin.Id, CategoryId = typeSkill.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = hookPlugin.Id, CategoryId = typeHook.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = agentPlugin.Id, CategoryId = typeAgent.Id });
        await ctx.SaveChangesAsync();
        await RefreshSearchVectors(ctx);
    ...
    public async Task SearchAsync_LanguageFilter_ReturnsOnlyMatchingLanguage()
    {
        ...
        AddOnEntity tsPlugin = MakePlugin("ValidationTs", "validation-ts",
            "TypeScript validation plugin");
        AddOnEntity pyPlugin = MakePlugin("ValidationPy", "validation-py",
    ...
    public async Task SearchAsync_TypeAndLanguageFilter_AndAcrossDimensionsIntersects()
    {
        ...
        AddOnEntity tsCommandPlugin = MakePlugin("TsCommandPlugin", "ts-command-and",
            "TypeScript command plugin for cross-dimension filter test");
        // command + Go → matches type but NOT language
        AddOnEntity goCommandPlugin = MakePlugin("GoCommandPlugin", "go-command-and",
            "Go command plugin for cross-dimension filter test");
        ctx.Plugins.AddRange(tsCommandPlugin, goCommandPlugin);
        await ctx.SaveChangesAsync();

        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = tsCommandPlugin.Id, CategoryId = typeCommand.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = tsCommandPlugin.Id, CategoryId = langTs.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = goCommandPlugin.Id, CategoryId = typeCommand.Id });
        ctx.PluginCategories.Add(new AddOnCategoryEntity { PluginId = goCommandPlugin.Id, CategoryId = langGo.Id });
        await ctx.SaveChangesAsync();
        await RefreshSearchVectors(ctx);

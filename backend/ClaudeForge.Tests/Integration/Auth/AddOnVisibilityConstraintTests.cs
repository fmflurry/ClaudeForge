...
/// OwnerOrgId, and OwnerUserId properties to AddOnEntity and creates the
...
public sealed class PluginVisibilityConstraintTests : IAsyncLifetime
{
    ...
    public async Task Insert_PublicPluginWithNullOwnerOrgId_Succeeds()
    {
        ...
        AddOnEntity plugin = MakePlugin("PublicOwnerlessPlugin", "public-ownerless-cv2");
        // Visibility = "public", OwnerOrgId = null — satisfies CHECK

        ctx.Plugins.Add(plugin);
        await ctx.SaveChangesAsync(); // Must NOT throw

        AddOnEntity? persisted = await ctx.Plugins.AsNoTracking().FirstOrDefaultAsync(p => p.Id == plugin.Id);
    ...
    public async Task Insert_PluginWithoutExplicitVisibility_DefaultsToPublic()
    {
        ...
        AddOnEntity? persisted = await ctx.Plugins.AsNoTracking().FirstOrDefaultAsync(p => p.Id == pluginId);
        Assert.NotNull(persisted);
        Assert.Equal("public", persisted!.Visibility);
        Assert.Null(persisted!.OwnerOrgId);
    ...
    public async Task ExistingPlugin_HasPublicVisibilityAndNullOwners_AfterMigration()
    {
        ...
        AddOnEntity legacy = MakePlugin("LegacyPlugin", "legacy-plugin-def2");
        ...
        AddOnEntity? persisted = await ctx.Plugins.AsNoTracking().FirstOrDefaultAsync(p => p.Id == legacy.Id);
        Assert.NotNull(persisted);
        Assert.Equal("public", persisted!.Visibility);
        Assert.Null(persisted!.OwnerOrgId);
        Assert.Null(persisted!.OwnerUserId);
    ...
    public async Task Query_PluginsByVisibilityAndOwnerOrgId_ReturnsCorrectResults()
    {
        ...
        AddOnEntity publicPlugin = MakePlugin("PublicIndexPlugin", "public-index-def3");

        AddOnEntity privatePlugin = new()
        {
            Id = Guid.NewGuid(),

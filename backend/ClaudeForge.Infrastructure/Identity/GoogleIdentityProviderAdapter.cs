...
/// <summary>
/// OIDC adapter for Google identity (accounts.google.com).
/// </summary>
public sealed class GoogleIdentityProviderAdapter : IIdentityProviderPort, INamedIdentityProviderPort
{
    ...
    private readonly ILogger<GoogleIdentityProviderAdapter>? _logger;
    ...
    public GoogleIdentityProviderAdapter(
        ...
        ILogger<GoogleIdentityProviderAdapter>? logger = null)
    {
        _clientId = configuration["OIDC__GOOGLE__CLIENTID"]
            ?? throw new InvalidOperationException("OIDC__GOOGLE__CLIENTID is not configured.");
        _clientSecret = configuration["OIDC__GOOGLE__CLIENTSECRET"]
            ?? throw new InvalidOperationException("OIDC__GOOGLE__CLIENTSECRET is not configured.");
    ...
    public string BuildAuthorizationUrl(
        ...
        string nonce = "")
    {
        // Build query string manually to control exact encoding (HttpUtility.UrlEncode for values)
        ...
        CancellationToken ct = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger?.LogWarning(
                "Google token exchange failed with status {StatusCode}. Response: {Body}",
        ...
        return tokenResponse?.IdToken
               ?? throw new InvalidOperationException("Google token response did not contain id_token.");
    }

  ...
  template: `
      ...
      <div class="lp-hero__inner">
        ...
        <div class="lp-hero__ctas" role="group" aria-label="Primary actions">
          ...
          <a
            ...
            aria-label="Learn how to publish a plugin"
          >
            Publish a plugin
          </a>
      ...
      <div class="lp-section-inner">
        ...
        @if (isLoadingPlugins()) {
          <div
            ...
            aria-busy="true"
            role="status"
            aria-label="Loading popular plugins"
          >
            Loading plugins…
          </div>
        }
        ...
        @if (!isLoadingPlugins() && !hasError() && featuredPlugins().length > 0) {
              ...
              <li class="lp-plugin-card">
                ...
                <div class="lp-plugin-card__meta">
                  ...
                  <span
                    ...
                    aria-label="{{ plugin.downloadCount }} downloads"
                  >
          ...
          <div class="lp-featured__cta">
            <a routerLink="/catalog" class="lp-btn lp-btn--secondary">
              View all plugins
            </a>
          </div>
        }
    ...
    <footer class="lp-footer" role="contentinfo">
      ...
      <p class="lp-footer__copy">
        &copy; {{ currentYear }} ClaudeForge — A plugin marketplace for Claude Code
      </p>
    </footer>
  `,
  styles: [
    `
      ...
      .lp-btn--primary { background: #1a1a2e; color: #fff; border-color: #1a1a2e; }
      .lp-btn--primary:hover { background: #16213e; border-color: #16213e; }
      ...
      .lp-hero { background: #1a1a2e; color: #fff; padding: 5rem 1.5rem 4rem; text-align: center; }
      .lp-hero__inner { max-width: 56rem; margin: 0 auto; }
      .lp-hero__title { font-size: clamp(2rem,5vw,3.25rem); font-weight: 800; line-height: 1.1; letter-spacing: -0.03em; margin: 0 0 1.25rem; }
      .lp-hero__brand { color: #818cf8; }
      .lp-hero__tagline { font-size: 1.125rem; color: rgba(255,255,255,.78); max-width: 40rem; margin: 0 auto 2.5rem; line-height: 1.7; }
      .lp-hero__ctas { display: flex; flex-wrap: wrap; gap: 0.875rem; justify-content: center; }
      .lp-hero .lp-btn--primary { background: #818cf8; border-color: #818cf8; color: #fff; }
      .lp-hero .lp-btn--primary:hover { background: #6366f1; border-color: #6366f1; }
      .lp-hero .lp-btn--secondary { color: #fff; border-color: rgba(255,255,255,.55); }
      .lp-hero .lp-btn--secondary:hover { background: rgba(255,255,255,.1); }

      .lp-search-entry { background: #f3f4f6; padding: 2rem 1.5rem; }
      .lp-search-entry__form { max-width: 48rem; margin: 0 auto; display: flex; gap: 0.625rem; }
      .lp-search-entry__input { flex:1; padding:.625rem 1rem; border:2px solid #d1d5db; border-radius:.375rem; font-size:.9375rem; background:#fff; color:#111827; transition:border-color .2s; outline:none; }
      .lp-search-entry__input:focus { border-color:#818cf8; box-shadow:0 0 0 3px rgba(129,140,248,.2); }

      .lp-featured { padding: 4rem 0; background: #fff; }
      .lp-featured__loading,
      .lp-featured__error { text-align: center; padding: 3rem 0; color: #6b7280; font-size: 0.9375rem; }
      .lp-featured__error { color: #b91c1c; }
      .lp-featured__grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(18rem,1fr)); gap: 1.5rem; list-style: none; margin: 0 0 2.5rem; padding: 0; }
      .lp-featured__cta { text-align: center; }

      .lp-plugin-card { border:1px solid #e5e7eb; border-radius:.75rem; padding:1.25rem; display:flex; flex-direction:column; gap:.75rem; background:#fff; transition:box-shadow .2s; }
      .lp-plugin-card:hover { box-shadow:0 4px 16px rgba(0,0,0,.08); }
      .lp-plugin-card__header { display: flex; align-items: flex-start; justify-content: space-between; gap: 0.5rem; }
      .lp-plugin-card__name { font-size: 1rem; font-weight: 700; color: #111827; line-height: 1.3; }
      .lp-plugin-card__description { font-size: 0.875rem; color: #4b5563; line-height: 1.6; margin: 0; overflow: hidden; max-height: 4.8em; }
      .lp-plugin-card__meta { display: flex; justify-content: space-between; font-size: 0.8125rem; color: #6b7280; gap: 0.5rem; flex-wrap: wrap; }
      .lp-plugin-card__tags { display: flex; flex-wrap: wrap; gap: 0.375rem; }

      .lp-how { padding: 4rem 0; background: #f9fafb; }
      .lp-how__steps { display: grid; grid-template-columns: repeat(auto-fill,minmax(14rem,1fr)); gap: 2rem; list-style: none; margin: 0; padding: 0; }
      .lp-how__step { display: flex; flex-direction: column; gap: 0.625rem; }
      .lp-how__icon { font-size: 2rem; line-height: 1; }
      .lp-how__step-title { font-size: 1.0625rem; font-weight: 700; color: #111827; margin: 0; }
      .lp-how__step-desc { font-size: 0.9375rem; color: #4b5563; line-height: 1.6; margin: 0; }

      .lp-footer { background: #1a1a2e; color: rgba(255,255,255,.75); padding: 2.5rem 1.5rem; text-align: center; }
      .lp-footer__nav { display: flex; flex-wrap: wrap; justify-content: center; gap: 0.25rem 1.5rem; margin-bottom: 1.25rem; }
      .lp-footer__link { color: rgba(255,255,255,.75); text-decoration: none; font-size: 0.9375rem; transition: color 0.2s ease; }
      .lp-footer__link:hover { color: #fff; }
      .lp-footer__link:focus-visible { outline: 2px solid #818cf8; outline-offset: 2px; border-radius: 2px; }
      .lp-footer__copy { font-size: 0.8125rem; margin: 0; color: rgba(255,255,255,.5); }

      @media (max-width: 640px) {
        .lp-hero { padding: 3rem 1rem 2.5rem; }
        .lp-hero__ctas { flex-direction: column; align-items: center; }
        .lp-search-entry__form { flex-direction: column; }
        .lp-featured__grid,
...
export class LandingPageComponent implements OnInit {
  ...
  readonly hasError: Signal<boolean> = computed(
    () => this.catalogFacade.pluginsError() !== undefined,
  );

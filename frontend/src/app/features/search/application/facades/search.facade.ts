...
import { buildSearchQueryParams, combineSearchFilters } from '../../domain/rules/search-filter.rules';
...
export class SearchFacade {
  ...
  search(keyword: string, filters?: Partial<SearchFilterQuery>): void {
    const merged = combineSearchFilters({ ...this._currentQuery(), keyword }, filters ?? {});
    this._currentQuery.set(merged);

    this.store.startLoading(SearchStoreEnum.SEARCH_RESULTS);

    this.port
      .search(buildSearchQueryParams(merged))
  ...
  setPage(page: number): void {
    ...
    this.port
      .search(buildSearchQueryParams(updated))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (resultPage) => {
          this._paginationMeta.set({
